...
namespace ufo
{
  ...
  public:
    SMTUtils(ExprFactory &_efac) : efac(_efac), z3(efac), smt(z3), can_get_model(0), m(NULL) {}

    SMTUtils(ExprFactory &_efac, unsigned _to) : efac(_efac), z3(efac), smt(z3, _to), can_get_model(0), m(NULL) {}
    ...
    template <typename T>
    void getOptModel(ExprSet &vars, ExprMap &e, Expr v)
    {
      if (!can_get_model)
        return;
      while (true)
    ...
    template <typename T>
    boost::tribool isSat(T &cnjs, bool reset = true)
    {
      if (m != NULL)
      {
        delete m;
    ...
    boost::tribool isSatIncrem(ExprVector &v, int &sz)
    {
      ...
      while (sz < v.size())
      {
        ...
        sz++;
        if (res == false || indeterminate(res))
          return res;
      }
      return true; // sat
    }
    ...
    Expr simplifyITE(Expr ex, Expr upLevelCond)
    {
      if (isOpX<ITE>(ex))
      {
        ...
        if (!isSat(cond, upLevelCond))
          return br2;

        if (!isSat(mk<NEG>(cond), upLevelCond))
          return br1;
    ...
    Expr removeITE(Expr ex)
    {
      ...
      if (sz == ites.size())
        ...
        return simplifyBool(simplifyArithm(removeITE(ex)));
    }
    ...
    void removeRedundantDisjuncts(Range &disjs)
    {
      if (disjs.size() < 2)
        return;
    ...
    Expr simplifiedAnd(Expr a, Expr b)
    {
      ...
      flatten(a, disjs, false, vars, [](Expr a, ExprVector &b)
              { return a; });
      for (auto it = disjs.begin(); it != disjs.end();)
      {
        if (!isSat(*it, b))
