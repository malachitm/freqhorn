...
namespace ufo
{
  ...
  public:
    RndLearner(ExprFactory &efac, EZ3 &z3, CHCs &r, unsigned to, bool k, bool b1, bool b2, bool b3, int debug) : m_efac(efac), m_z3(z3), ruleManager(r), m_smt_solver(z3), u(efac, to),
    ...
    void reportCheckingResults(bool doRedundancyOptim = true)
    {
      for (int i = 0; i < invNumber; i++)
      {
        ...
        {
          if (printLog)
            outs() << "    => learned lemma for " << *decls[i] << "\n";
    ...
    {
      for (auto &sf : sfs)
        sf.back().learnedExprs.clear();
    }

    bool checkWithKInduction()
    {
      if (ruleManager.chcs.size() != 3)
        return false; // current limitation
      if (sfs.size() != 1)
        return false; // current limitation
      if (kind_succeeded)
        return false;
      ...
      if (kind_succeeded)
      {
        ...
        if (oneInductiveProof) // can complete the invariant only when the proof is 1-inductive
        {
          ...
          if (addedRemainingLemma)
            sf.learnedExprs.insert(curCandidates[0]); // for serialization only

          if (printLog)
            outs() << "remaining lemma(s): " << *curCandidates[0] << "\nsanity check: " << addedRemainingLemma << "\n";
        }
      }
    ...
    bool checkBoundedProofs(ExprSet &itpCandidates)
    {
      ...
      for (auto it = itpCandidates.begin(), end = itpCandidates.end(); it != end;)
      {
        ...
        if (printLog)
          outs() << "itp candidate for " << *decls[0] << ": " << **it << "\n";
    ...
    void updateRels()
    {
        ...
        if (!isOpX<TRUE>(cand))
        {
          for (auto &hr : ruleManager.chcs)
          {
                ...
                !hr.isQuery)
            {
              ...
              numOfSMTChecks++;
              if (u.implies(hr.body, lemma2add))
                continue;
      ...
      for (auto ind : rels2update)
      {
        ...
        for (auto &var : invarVars[ind])
          sf_after.addVar(var.second);
    ...
    virtual void doSeedMining(Expr invRel, ExprSet &cands, set<cpp_int> &progConsts, set<cpp_int> &intCoefs)
    {
      ...
      for (auto &hr : ruleManager.chcs)
      {
        ...
        for (auto &a : sm.intCoefs)
          intCoefs.insert(a); // same for intCoefs
        for (auto &a : sm.candidates)
          cands.insert(a);
      }
    }
    ...
    void synthesize(int maxAttempts, ExprSet &itpCands)
    {
      ...
      for (int i = 0; i < maxAttempts; i++)
      {
        ...
        {
          if (curCandidates[j] != NULL)
            continue; // if the current candidate is good enough
        ...
        if (isInductive)
          success = checkSafety(); // query is checked here
        ...
        for (auto &cand : curCandidates)
          cand = NULL; // preparing for the next iteration
      }

      if (success)
        outs() << "Success after " << --iter << " iterations\n";
      else
        outs() << "Unknown after " << maxAttempts << " iterations\n";
    ...
    void checkAllLemmas(vector<ExprSet> &lms, vector<ExprSet> &curMinLms, int &numTries, int invInd)
    {
      ...
      resetlearnedLemmas();
      for (int i = 0; i < invNumber; i++)
        curCandidates[i] = conjoin(lms[i], m_efac);

      if (checkCandidates() && checkSafety())
      {
        if (lms[invInd].size() < curMinLms[invInd].size())
          curMinLms[invInd] = lms[invInd];
