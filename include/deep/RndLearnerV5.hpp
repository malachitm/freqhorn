...
namespace ufo
{
    ...
    public:
        ...
        /// Build the assertion set for a single CHC check, mirroring
        /// the logic of checkCHC2 but returning the expression set
        /// instead of calling the solver.
        ExprSet buildCHCExprs(HornRuleExt &hr, map<int, ExprVector> &annotations)
        {
            int srcNum = getVarIndex(hr.srcRelation, decls);
            int dstNum = getVarIndex(hr.dstRelation, decls);
        ...
        /// Generate an SMT-LIB2 certificate file containing three
        /// sequential (push)/(pop) queries for Fact, Consecution, and
        /// Query checks.  A correct inductive invariant yields
        ///   unsat\n unsat\n unsat
        /// when evaluated by any compliant SMT solver.
        void generateCertificate(const std::string &filename, int invNum,
                                 map<int, ExprVector> &annotations)
        {
            assert(invNum < invNumber);
            std::ofstream smtFile(filename);
            if (!smtFile.is_open())
            {
                errs() << "Error: Unable to open certificate file " << filename << "\n";
                return;
            }

            // Force rational printing so literals are (/ p q) not decimals
            Z3_global_param_set("pp.decimal", "false");

            smtFile << "; SMT-LIB2 inductive-invariant certificate\n";
            smtFile << "; Generated by FreqHorn\n";
            smtFile << "(set-logic QF_NRA)\n\n";

            // Helper lambda: emit one (push 1) block for a CHC check
            auto emitQuery = [&](const char *tag, HornRuleExt &hr)
            {
                ExprSet rawExprs = buildCHCExprs(hr, annotations);

                // Convert MPZ leaves -> MPQ so Z3 marshals Real sort
                ExprVector assertVec;
                assertVec.reserve(rawExprs.size());
                for (auto &e : rawExprs)
            ...
            emitQuery("Fact (Initiation)", *fc[invNum]);
            emitQuery("Consecution (Step)", *tr[invNum]);
            emitQuery("Query (Safety)", *qr[invNum]);
            ...
            outs() << "Certificate written to " << filename << "\n";
        }
