...
#define FREE_LIST_MAX_SIZE 1024 * 4
  ...
  inline ENode::ENode(ExprFactory &f, const Operator &o) : count(0), fac(&f),
...
namespace expr
{
  ...
  public:
    ...
    bool operator<(const Operator &rhs) const
    {
      ...
      return (prhs == NULL) ? typeid(this_type).before(typeid(rhs)) : terminal_type::less(val, prhs->val);
    }
  ...
  template <>
  struct TerminalTrait<std::string>
  {
  ...
  template <>
  struct TerminalTrait<unsigned int>
  {
  ...
  template <>
  struct TerminalTrait<unsigned long>
  {
    static inline void print(std::ostream &OS,
  ...
  /** An algebraic number: a root of a univariate integer polynomial.
   *  Stores the defining polynomial coefficients [c_0, c_1, ..., c_n]
   *  (i.e. c_0 + c_1*x + ... + c_n*x^n = 0), the root index that
   *  selects which root of the polynomial this represents, and an
   *  isolating rational interval [lower, upper] for quick approximation.
   */
  struct AlgebraicNum
  {
    mpq_class lower;             ///< lower bound of isolating interval
    mpq_class upper;             ///< upper bound of isolating interval
    std::vector<mpz_class> poly; ///< integer polynomial coefficients
    unsigned rootIdx;            ///< which root (0-based)

    /// Rational midpoint of the isolating interval.
    mpq_class midpoint() const
    {
      mpq_class mid = (lower + upper) / 2;
      mid.canonicalize();
      return mid;
    }

    /// Double-precision approximation (midpoint).
    double to_double() const { return midpoint().get_d(); }

    /// True when the polynomial has degree <= 1 (value is rational).
    bool isRational() const { return poly.size() <= 2; }

    /// Degree of the defining polynomial.
    unsigned degree() const { return poly.empty() ? 0 : (unsigned)(poly.size() - 1); }

    bool operator==(const AlgebraicNum &o) const
    {
      return poly == o.poly && rootIdx == o.rootIdx;
    }
  ...
  template <>
  struct TerminalTrait<AlgebraicNum>
  {
    static inline void print(std::ostream &OS, const AlgebraicNum &v,
                             int depth, bool brkt)
    {
      OS << "(root-obj [";
      ...
      OS << "] " << v.rootIdx
         << " ~" << std::setprecision(15) << v.midpoint().get_d() << ")";
    }

    static inline bool less(const AlgebraicNum &a, const AlgebraicNum &b)
    {
      return a < b;
    }

    static inline bool equal_to(const AlgebraicNum &a, const AlgebraicNum &b)
    {
      return a == b;
    }

    static inline size_t hash(const AlgebraicNum &v)
    {
      size_t h = std::hash<unsigned>()(v.rootIdx);
      for (auto &c : v.poly)
      {
  ...
  namespace op
  {
    ...
    typedef Terminal<AlgebraicNum> ALNUM;
  }
  ...
  public:
    // skipKids or doKids
    VisitAction(bool kids = false) : _skipKids(kids), fn(new ExprFunctionoid<IdentityRewriter>(std::make_shared<IdentityRewriter>())) {}

    // changeTo or doKidsRewrite
    template <typename R>
    VisitAction(Expr e, bool kids = false,
                std::shared_ptr<R> r = std::make_shared<IdentityRewriter>()) : _skipKids(kids), expr(e), fn(new ExprFunctionoid<R>(r)) {}
  ...
  namespace op
  {
    ...
    namespace boolop
    {
      ...
      struct TrivialSimplifier : public std::unary_function<Expr, Expr>
      {
        ...
        TrivialSimplifier(const TrivialSimplifier &o) : efac(o.efac), trueE(o.trueE), falseE(o.falseE) {}

        TrivialSimplifier(ExprFactory &fac) : efac(fac), trueE(mk<TRUE>(efac)), falseE(mk<FALSE>(efac)) {}
      ...
      struct NNF : public std::unary_function<Expr, VisitAction>
      {
        ...
        VisitAction operator()(Expr exp)
        {
          ...
          if (isOpX<OR>(lhs) || isOpX<AND>(lhs))
          {
            ...
            // -- flip operator
            Expr res = isOpX<OR>(lhs) ? mknary<AND>(args.begin(), args.end()) : mknary<OR>(args.begin(), args.end());
...
namespace expr
{
  ...
  template <>
  struct TerminalTrait<op::bind::BoundVar>
  {
    static inline void print(std::ostream &OS,
  ...
  namespace
  {
    ...
    struct RAVALL : public std::unary_function<Expr, VisitAction>
    {
      ...
      VisitAction operator()(Expr exp) const
      {
        ...
        for (unsigned int i = 0; i < sz; i++)
          if (exp == s->at(i))
            return VisitAction::changeTo(t->at(i));
        return VisitAction::doKids();
      }
    };
    ...
    struct RAVSIMP : public std::unary_function<Expr, VisitAction>
    {
      ...
      RAVSIMP(Expr _s, Expr _t) : s(_s), t(_t), r(std::make_shared<boolop::TrivialSimplifier>(s->efac())) {}
    ...
    struct RV : public std::unary_function<Expr, VisitAction>
    {
      ...
      VisitAction operator()(Expr exp) const
      {
        ...
        return it == map.end() ? VisitAction::doKids() : VisitAction::changeTo(it->second);
      }
    };
    ...
    struct RVSIMP : public std::unary_function<Expr, VisitAction>
    {
      ...
      RVSIMP(ExprFactory &fac, const M &m) : map(m),
  ...
  template <typename M>
  inline bool containsOp(Expr e1)
  {
    ContainsOp<M> co;
    dagVisit(co, e1);
    return co.found;
  ...
  namespace op
  {
    namespace bind
    {
      ...
      inline Expr typeOf(Expr v)
      {
        ...
        if (isOpX<SELECT>(v))
          return typeOf(v->left())->last();
        if (isOpX<CONST_ARRAY>(v))
          return sort::arrayTy(v->left(), typeOf(v->right()));
...
namespace expr
{
  namespace details
  {
    ...
    DagVisit<ABSCST<AbsCst<Range>>> &AbsCst<Range>::cachedVisitor(unsigned offset)
    {
      ...
      auto v = m_cache.insert(std::make_pair(offset, DagVisit<ABSCST<this_type>>(m_pinned.back())));
    ...
    DagVisit<SUBBND<SubBnd<Range>>> &SubBnd<Range>::cachedVisitor(unsigned offset)
    {
      ...
      auto v = m_cache.insert(std::make_pair(offset, DagVisit<SUBBND<this_type>>(m_pinned.back())));
      return (v.first)->second;
    }
