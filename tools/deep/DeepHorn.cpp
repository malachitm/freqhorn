...
int getIntValue(const char *opt, int defValue, int argc, char **argv)
{
  for (int i = 1; i < argc - 1; i++)
  {
    if (strcmp(argv[i], opt) == 0)
    {
      ...
      if (*p)
        return 1; // if used w/o arg, return boolean
      else
        return num;
    }
  }
...
int main(int argc, char **argv)
{
  ...
  {
    outs() << "* * *                                 FreqHorn v.0.6 - Copyright (C) 2021                                 * * *\n"
           ...
           << OPT_V3 << " (default)                  optimized version (+ bootstrapping, propagation, and data candidates)\n"
                        " "
           << OPT_V4 << "                            optimized version (+ MBP-based, path-sensitive)\n"
                        " "
           << OPT_V5 << "                            optimized version for affine systems\n"
           << "  "
  ...
  if (!vers1 && !vers2 && !vers3 && !vers4 && !vers5)
    vers4 = true; // default
  ...
  if (do_disj)
  {
    ...
    {
      if (debug)
        errs() << "WARNING: \"" << OPT_SEED << "\" and \"" << OPT_DISJ << "\" are incompatible. "
               << "Ignoring \"" << OPT_SEED << "\".\n";
      d_se = true;
    }
    if (do_prop == 0)
